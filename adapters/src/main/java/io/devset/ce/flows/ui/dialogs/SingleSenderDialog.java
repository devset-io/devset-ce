...
@UtilityClass
public class SingleSenderDialog {

    public static Optional<RuleDto> showDialog(RuleDto ruleDto, String schemaTitle, List<FieldRuleDto> defaults, ProviderMetaData providerMetadata) {
        Dialog<RuleDto> dialog = createDialog(schemaTitle, "Single sender");
    ...
    private static TableView<FieldRuleDto> createFieldsTableView(ObservableList<FieldRuleDto> selectedFields) {
        ...
        TableColumn<FieldRuleDto, String> defaultCel = createDefaultNameColumn("value");
        TableColumn<FieldRuleDto, Void> removeCol = createRemoveColumn(selectedFields);

        table.getColumns().addAll(fieldNameCol, typeCol, defaultCel, removeCol);
        return table;
    }
