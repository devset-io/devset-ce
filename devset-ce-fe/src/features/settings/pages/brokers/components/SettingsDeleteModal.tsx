...
export const SettingsDeleteModal = React.memo(function SettingsDeleteModal({
  ...
  return (
    <ModalShell
      ...
      title={t('settings.confirm.deleteTitle')}
      subtitle={t('settings.confirm.deleteSubtitle').replace('{name}', connectorPendingDelete?.name ?? '')}
    ...
    >
      <div className="settings-modal-copy">
        <p className="settings-modal-lead">{t('settings.confirm.deleteBody')}</p>
      </div>
      <footer className="flex items-center justify-end gap-2">
        ...
        <button
          ...
          disabled={isSubmitting}
        >
          {isSubmitting ? t('settings.action.deleting') : t('settings.action.confirmDelete')}
        </button>
      </footer>
    </ModalShell>
  )
