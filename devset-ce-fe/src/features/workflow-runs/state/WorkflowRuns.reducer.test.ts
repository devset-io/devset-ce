...
import { createInitialState, reducer } from './WorkflowRuns.reducer'
import type { WorkflowRunsState } from './WorkflowRuns.types'

const stateWith = (overrides: Partial<WorkflowRunsState>): WorkflowRunsState => ({
  ...createInitialState(),
  ...overrides,
})
...
describe('connectorSelected', () => {
  it('clears topicSuggestions and exchangeSuggestions when connector changes', () => {
    const state = stateWith({
      ...
      topicSuggestions: ['topic-a', 'topic-b'],
      exchangeSuggestions: ['exchange-a'],
    })
    const next = reducer(state, { type: 'connectorSelected', name: 'new' })
  ...
  it('returns same state when re-selecting the same connector', () => {
    ...
    const next = reducer(state, { type: 'connectorSelected', name: 'same' })
