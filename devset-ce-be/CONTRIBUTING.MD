# Contributing to Devset CE (Backend)
...
- Gradle 9.4+ (wrapper included — use `./gradlew`)
- protoc 34.1 (required for Protobuf schema compilation)
...
- Onion Architecture — see the architecture guide for details
- Lombok (`@RequiredArgsConstructor`, `@Slf4j`) — mandatory
- MapStruct for all layer-boundary mappings
...
- [ ] `./gradlew build` passes (compiles, tests, checks)
...
Open a [Discussion](https://github.com/devset-io/devset-ce-be/discussions)
